...
export const $SearchSummoners = createServerFn({ method: "GET" })
  ...
  .handler(async (ctx) => {
    ...
    const data = await SummonerService.searchSummoners({
      userId,
      search: ctx.data.c,
      limit: 5,
    });
...
export type $SearchSummonersType = Awaited<ReturnType<typeof $SearchSummoners>>;
