...
import { $getAllSummonerMatchesByRiotID } from "@/server/functions/$getAllSummonerMatchesByRiotID";
...
export const Route = createFileRoute("/lol/summoner/$riotID/charts")({
  ...
  loader: async (context) => {
    const matches = await $getAllSummonerMatchesByRiotID({
      data: {
        riotId: context.params.riotID,
    ...
    return {
      data: matches.matches.map<MatchPoint>((m) => {
        return {
          datetime: new Date(m.match.gameCreationMs),
          win: m.match_summoner.win,
        };
      }),
      matches: matches.matches.map<MatchRow>((m) => {
        return {
          id: m.match.matchId,
          datetime: new Date(m.match.gameCreationMs),
          duration: m.match.gameDurationSec,
          championId: m.match_summoner.championId,
          win: m.match_summoner.win,
          kills: m.match_summoner.kills,
          deaths: m.match_summoner.deaths,
          assists: m.match_summoner.assists,
        };
      }),
    };
  },
...
function RouteComponent() {
  const { data, matches } = Route.useLoaderData();
    ...
    <div className="w-full mx-auto">
      <div className="grid grid-cols-12 gap-5 relative">
        ...
        <div className="col-span-5 h-[var(--summoner-outlet-height)]">
          <ChartsMatchesVirtualTable data={matches} />
        </div>
